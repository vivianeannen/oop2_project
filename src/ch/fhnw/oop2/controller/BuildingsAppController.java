...
public class BuildingsAppController implements Initializable {
	...
	public void setBuildings(ObservableList<BuildingPM> buildingsData) {
		...
		SortedList<BuildingPM> sortedData = new SortedList<>(filteredData);
		sortedData.setComparator((o1, o2) -> (int) (Double.valueOf(o2.getHeightM()) - Double.valueOf(o1.getHeightM())));
		sortedData.comparatorProperty().bind(tvBuildings.comparatorProperty());

		tvBuildings.setItems(sortedData);
	}
	...
	private void setRanking() {
		Platform.runLater(() -> {
			buildings.getBuildingsData().sort((o1, o2) -> (int) (Double.valueOf(o2.getHeightM()) - Double.valueOf(o1.getHeightM())));
			for (int i = 0; i < buildings.getBuildingsData().size(); i++) {
				buildings.getBuildingsData().get(i).setRank("" + (i + 1));
			}
		});
	}
}
